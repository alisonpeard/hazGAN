...
# gumbel transform functions -- Alison 2024-01-20
def optimal_K(x):
    with torch.no_grad():
        sample_points = x
...
def gumbel_transform(x, K=None, eps=1e-6):
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        # # ------------------------
        # # Gumbel transformations
        # # ------------------------
        # thresh = torch.rand([batch_size, 1, 1, 1, 1], device=device)
        # x = torch.where(thresh < self.gumbel_K * self.p, gumbel_transform(images), images)
