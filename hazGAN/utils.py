...
def classify_pressure(pressure):
    """Classify by storm lifetime minimum SLP https://doi.org/10.5194/gmd-15-6759-2022"""
    def classify(pressure):
        if pressure < 92500:
            return "5"
        elif pressure < 94500:
            return "4"
            ...
            return "no storm"
    return np.vectorize(classify)(pressure)
    

def classify_wind(wind):
    """Classify by storm lifetime maximum wind speed https://doi.org/10.5194/gmd-15-6759-2022"""
    def classify(wind):
        if wind >= 63:
            return "5"
        elif wind >= 52:
            ...
            return "no storm"
    return np.vectorize(classify)(wind)