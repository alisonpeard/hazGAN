...
def plot(fake, real, field=0, pixels=None, cmap=CMAP, s=10,
         xlabel=None, ylabel=None, figsize=(6, 3), suptitle=''):
        _, h, w, c = real.shape
        if pixels is None:
        ...
        single_scatter(real[..., field], ax=axs[0], sample_pixels=[j,i], cmap=cmap, s=s)
        single_scatter(fake[..., field], ax=axs[1], sample_pixels=[j,i], cmap=cmap, s=s)

        axs[0].set_title(f"Data (n={len(real)})")
        axs[1].set_title(f"Samples (n={len(fake)})")

        for ax in axs:
            if ylabel:
                ax.set_ylabel(ylabel)
            else:
                ax.set_ylabel(f"pixel index {i}")

            if xlabel:
                ax.set_xlabel(xlabel)
            else:
                ax.set_xlabel(f"pixel index {j}")
