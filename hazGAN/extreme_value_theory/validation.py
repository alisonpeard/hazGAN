...
@tf.function
def chi_loss(real, fake, gumbel_margins=False):
    """
    ...
    """
    if gumbel_margins:
        # convert to uniform --added stability clipping
        real = tf.clip_by_value(inv_gumbel(real), 1e-6, 1 - 1e-6)
        fake = tf.clip_by_value(inv_gumbel(fake), 1e-6, 1 - 1e-6)
    c = tf.shape(real)[-1]
